...
public class ImageCache {
	...
	public static final int DEFAULT_MEM_CACHE_SIZE = 1024 * 1024 * 4; // 4MB
	 ...
	 */
	public static void setup(Context ctx, String pathName) {
		ImageCache.CACHE_DIR_IMAGES = pathName;
		DiskLruCache.mCacheDir = DiskLruCache.getDiskCacheDir(ctx, ImageCache.CACHE_DIR_IMAGES);
		if (DiskLruCache.mCacheDir != null) {
			if (!DiskLruCache.mCacheDir.exists()) {
	...
	public static ImageCache getInstance(Context context) {
		int fitSize = Utils.getMemCacheSize(context);
		ImageCacheParams cacheParams = new ImageCacheParams(CACHE_DIR_IMAGES);
		if (fitSize > 0) {
			cacheParams.memCacheSize = fitSize;
		}
		return new ImageCache(context, cacheParams);
	}

	public static ImageCache getInstance(FragmentActivity activity) {
		return findOrCreateCache(activity, CACHE_DIR_IMAGES);
	}
	...
	public static ImageCache findOrCreateCache(final FragmentActivity activity, final String uniqueName) {
		ImageCacheParams cacheParams = new ImageCacheParams(uniqueName);
		int fitSize = Utils.getMemCacheSize(activity);
		if (fitSize > 0) {
			cacheParams.memCacheSize = fitSize;
		}
		return findOrCreateCache(activity, cacheParams);
	}
	...
	public static ImageCache findOrCreateCache(final FragmentActivity activity, ImageCacheParams cacheParams) {
		...
		if (mRetainFragment != null) {
			...
			imageCache = (ImageCache) mRetainFragment.getObject();
		}
	...
	/**
	 * Try to find and return an existing ImageCache stored in a
	 * {@link RetainFragment}
	 * 
	 * @param activity
	 * @return ImageCache or null
	 */
	public static ImageCache tryTofindCache(final FragmentActivity activity) {
		// Search for, or create an instance of the non-UI RetainFragment
		RetainFragment mRetainFragment = (RetainFragment) activity.getSupportFragmentManager().findFragmentByTag(RetainFragment.TAG);
	 ...
	 *            Unique identifier for which item to get
	 * @return The bitmap if found in cache, null otherwise
	 * @deprecated never do that. Cause' return the original image .
	 */
	...
	public String getCacheFile(String data) {
		if (mDiskCache != null) {
			return mDiskCache.getCacheFile(data);
		}
		return null;
	}
